...
func (g *genericRun) Run(ctx context.Context) error {
	conf := map[string]any{
		"Iterations":                    g.config.Iterations,
		"StartFromIteration":            g.config.StartFromIteration,
		"Duration":                      g.config.Duration.String(),
		"MaxConcurrent":                 g.config.MaxConcurrent,
		"MaxIterationsPerSecond":        g.config.MaxIterationsPerSecond,
		"Timeout":                       g.config.Timeout,
		"DoNotRegisterSearchAttributes": g.config.DoNotRegisterSearchAttributes,
	}
	for k, v := range conf {
		g.logger.Debugw("CONFIG %s=%v", k, v)
	}
	return nil
